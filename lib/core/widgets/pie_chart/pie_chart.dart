...
import 'package:my_template/core/widgets/pie_chart/pie_data.dart';

///環形圖 參考：https://github.com/apgapg/pie_chart
...
class PieChartPainter extends CustomPainter {
  PieChartPainter(
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      // 排序後的數據輸出
//      print(data[i].function toString() { [native code] }());
    }
    if (pieData != null) {
      data.add(pieData);
    }
  ...
  void drawPercentage(
    ...
    final TextSpan span = TextSpan(
        style: const TextStyle(color: Colors.white, fontSize: Dimens.font_p12),
        text: percentage);
  ...
  bool shouldRepaint(PieChartPainter oldDelegate) {
    // 由於動畫需要重繪，所以返true。避免重繪，交由RepaintBoundary處理。你也可以判斷動畫是否執行完成來處理時候重繪
    return true;
  }
