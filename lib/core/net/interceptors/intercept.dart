...
import 'package:my_template/core/utils/other_utils.dart';
...
class TokenInterceptor extends QueuedInterceptor {
  ...
  Future<void> onResponse(
    ...
    if (response.statusCode == ExceptionHandle.unauthorized) {
      Log.d('-----------自動刷新Token------------');
      final String? accessToken = await getToken(); // 獲取新的accessToken
...
class AdapterInterceptor extends Interceptor {
  ...
  static const String _kDefaultText = '無返回信息';
  static const String _kNotFound = '未找到查詢信息';
...
//            Log.d('異常信息：$e');
            // 解析異常直接按照返回原數據處理（一般為返回500,503 HTML頁面代碼）
            result = sprintf(_kFailureFormat,
                [response.statusCode, '服務器異常(${response.statusCode})']);
          }
        }
      }
    }
